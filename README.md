...
* **`xxkb-x11`.** Помечен в todo как `in_progress` — основная
  функциональность есть. RandR hot-plug / hot-unplug разруливается
  через чистый планировщик `xxkb_core::monitors::reconcile_main_
  indicators` (9 unit-тестов: initial placement, hot-unplug,
  hot-plug, mode flip в обе стороны, смена primary, master-
  disable, steady-state no-op, set-based ordering). Не покрыты
  edge-кейсы: нестандартные WM-ы без `_NET_FRAME_EXTENTS`,
  EWMH-fallback на старых KWin / Mutter, dual-XInput с
  duplicated keyboards.

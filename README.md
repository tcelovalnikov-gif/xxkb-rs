...
* **Главный индикатор.** Cycle, save-position и **режимы отображения**
  (`primary_only` / `all_displays`) реализованы и юнит-тестируются:
  `place_initial_main_indicators` сначала уничтожает stale-индикаторы
  (mode flip, output unplug, master-disable), затем создаёт нужные.
  `repaint` использует список реально живых окон с сервера, без
  попыток рисовать на отсутствующих outputs. Save-dialog при Ctrl+drag
  сейчас просто пишет позицию в TOML без подтверждения — GTK
  message-dialog ещё не подключён.

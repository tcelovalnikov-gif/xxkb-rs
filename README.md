...
> **Текущий релиз: v0.2.0** (см. [CHANGELOG.md](CHANGELOG.md)). Ядро и CI
> стабильны; Wayland нативно и часть полировки ТЗ — в планах. Подробности —
...
* **Главный индикатор: cycle-by-click + save position on drag.** Демон
  ...
  пишет TOML. Опционально `main_indicator.confirm_drag_save` + `zenity`/
  `kdialog`; после успешного сохранения эмитится `PositionsSaved(1)`.
* **GUI-конфигуратор** (`xxkb-config`) на GTK4 + libadwaita. Все
  ...
  Фоновый слушатель `PositionsSaved` показывает тост, когда демон
  сохранил позицию с Ctrl+drag (или через D-Bus).
  Реальная бизнес-логика (валидация, dirty-tracking, save/discard) —
  в `xxkb-config-state` и **покрыта юнит-тестами**.
...
* **Docker DE-smoke матрица.** `tests/docker/Dockerfile.{xfce,mate,
  ...
  `xdotool key alt+shift`. CI-job `smoke-de` гейтован на
  `schedule`/`workflow_dispatch` (в т.ч. ручной запуск workflow
  **CI** на GitHub); каждый образ тянет ~600 МБ DE-пакетов.
...
* **Главный индикатор.** Cycle, save-position, **режимы отображения**
  ...
  реализованы. Опциональное подтверждение перед записью
  (`confirm_drag_save` + `zenity`/`kdialog`) и тост в конфигураторе
  по `PositionsSaved`.
* **`xxkb-migrate`.** Парсер legacy X-resources + CLI; **13+ unit-тестов**
  и **файловые фикстуры** `tests/fixtures/*.xxkbrc` с интеграционными
  тестами round-trip в TOML.

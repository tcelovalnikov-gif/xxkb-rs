...
* **Packaging.** `assets/desktop/xxkbd.desktop` (autostart) +
  ...
  ungated в CI.
* **Docker DE-smoke матрица.** `tests/docker/Dockerfile.{xfce,mate,
  lxde}` + общий [`smoke.sh`](tests/docker/smoke.sh) ставят
  `.deb` поверх runtime-стека целевого DE (xfwm4 / marco /
  openbox + панель) на Ubuntu 24.04, поднимают Xvfb + dbus-
  session + двухгрупповой `setxkbmap us,ru`, запускают `xxkbd`,
  проверяют 48×48 override-redirect окно и переживают
  `xdotool key alt+shift`. CI-job `smoke-de` остаётся гейтован
  на `schedule`/`workflow_dispatch` (каждый образ тянет ~600 МБ
  DE-пакетов и не нужен на каждый push).

### 🟡 Частично

* **`xxkb-x11`.** Все известные edge-кейсы покрыты:
  * RandR hot-plug / hot-unplug: чистый планировщик
    `xxkb_core::monitors::reconcile_main_indicators` (9
    unit-тестов: initial placement, hot-unplug, hot-plug, mode
    flip в обе стороны, смена primary, master-disable,
    steady-state no-op, set-based ordering).
  * EWMH-fallback для WM-ов без `_NET_FRAME_EXTENTS`:
    `xxkb_core::placement::FrameExtents::from_frame_and_client`
    + parent-walk через `QueryTree` в `xxkb_x11::tracker`
    (4 unit-теста: типичная Mutter-декорация, undecorated,
    clamp на отрицательные дельты, асимметричная декорация).
  * Не покрыты экзотические сетапы (dual-XInput с
    duplicated keyboards, пустой XKB-state-notify на
    первой XGrabKeyboard) — оставлены до отдельного запроса.
* **Главный индикатор.** Cycle, save-position, **режимы отображения**
  (`primary_only` / `all_displays`) и **multi-monitor reconcile** —
  все реализованы. `place_initial_main_indicators` теперь
  делегирует pure-функции `reconcile_main_indicators(existing,
  want) → MainIndicatorPlan { to_remove, to_place }` и не
  трогает индикаторы, которые уже корректны (no flicker в steady
  state). Save-dialog при Ctrl+drag сейчас просто пишет позицию
  в TOML без подтверждения — GTK message-dialog ещё не подключён.
* **`xxkb-migrate`.** ~350 строк парсера legacy X-resources формата +
  CLI. **13 unit-тестов** покрывают minimal/full xxkbrc, правила
  app_list (`ignore`/`start_alt`/`alt_groupN`), continuation-строки,
  inline-комментарии, named-colour маппинг, граничные случаи (empty
  input, unknown keys, `bell.enable: no` → `SoundMode::Off`,
  `border.width: 0` → disabled border) и end-to-end round-trip
  через TOML. На реальных «живых» конфигах из upstream-репозитория
  ещё не прогонялся.
...
* **`#06` — Display indicator: save-dialog после drag.** Режимы
  `primary_only` / `all_displays` ✅. Нужно ещё спрашивать «сохранить
  новую позицию?» через GTK message-dialog после drop-а драга
  (сейчас сохраняется молча).
* **`#16` — Migration tool: real fixtures.** Базовые тесты
  (13 шт) покрывают синтетические inputs. Нужно прогнать
  переход на 2-3 реальных `~/.xxkbrc` из upstream-репозитория и
  ручных пользовательских конфигов и зафиксировать regression
  fixture'ы.

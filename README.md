...
* **D-Bus-скелет.** Интерфейс `org.xxkb.Daemon1` определён
  (`Reload`, `GetMonitors`, `GetActiveWindows`, `SaveCurrentPositions`,
  сигнал `LayoutChanged`), `DaemonHandle` в `xxkbd` отвечает на все
  методы реальными данными (включая `WM_CLASS`/`WM_NAME` через
  `props_cache`). D-Bus-клиент в `xxkb-config-state` (async + blocking).
...
* **Главный индикатор.** Cycle и save-position работают, но **режимы
  ...
  `all_displays`.
* **D-Bus.** Все методы отвечают, но сигнал `LayoutChanged` пока
  **не эмитится** в `xxkb-daemon` (тип определён, эмиттер не
  подключён). Интеграционного теста на полный round-trip
  «GUI → Reload → daemon перечитал TOML» нет.
* **`xxkb-sound`.** Чистая политика «когда играть»
  (`should_play(SoundMode, Trigger) -> bool`) реализована и
  оттестирована (4 теста). Но `RodioPlayer` сидит за фичей
  `rodio-playback`, и в `xxkbd` сейчас используется `MockPlayer` —
  то есть **звук в реальном бинарнике пока не звучит**.
...
* **`#08` — Sound: подключить `RodioPlayer`** в `xxkbd`, прокидывать
  `Trigger` (Manual/Auto) из `LayoutChanged`-евентов, читать `sound.file`
  из конфига.
* **`#10` — D-Bus: `LayoutChanged`-сигнал.** Эмиттить из
  `event_loop` при каждой смене группы. Плюс интеграционный тест с
  реальным session-bus в CI (`dbus-run-session`).
...
* **`#15` — Packaging.** `cargo-deb` метаданные расставлены, но
  `assets/desktop/xxkb-config.desktop` и systemd user-unit
  (`xxkbd.service`) ещё не написаны. Autostart через
  `~/.config/autostart/xxkbd.desktop` тоже.
* **`#16` — Migration tool:** написать тесты на реальные конфиги (есть
  пара примеров в репо upstream, плюс распространённые случаи с
  `app_list.*`).
* **`#17` — Документация.** Нужны `docs/ARCHITECTURE.md`,
  `docs/CONFIG.md` (полный референс TOML-полей с примерами),
  `docs/COMPATIBILITY.md` (матрица DE/WM × фичи), `docs/MANUAL_TEST.md`
  (чек-лист ручного тестирования на реальном железе). README ссылается
  на эти файлы, но самих файлов ещё нет.
